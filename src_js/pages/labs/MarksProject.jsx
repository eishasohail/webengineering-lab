...
import { ChevronUp, ChevronDown, Search, Download, GraduationCap, Sparkles, Shield, UserPlus, ArrowUpDown, CheckCircle2 } from "lucide-react";
...
export default function MarksProject() {
    ...
    return (<ProjectPageLayout title="Performance Portal" subtitle="Project #05: Data table with sort and filter." maxWidth="max-w-7xl" showCard={false} containerClassName="bg-slate-50/50" backHoverColor="hover:text-slate-800">
            ...
            { label: "Enrolled", val: STUDENTS.length, suffix: " Active", icon: <UserPlus size={18}/>, color: "text-slate-500", bg: "bg-slate-50" }
        ].map((stat, i) => (<div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-6 group hover:shadow-xl hover:shadow-slate-200/40 transition-all cursor-default">
                            <motion.div whileHover={{ rotate: 15, scale: 1.15, rotateY: 20 }} transition={{ type: "spring", stiffness: 300 }} className={`w-12 h-12 ${stat.bg} ${stat.color} rounded-2xl flex items-center justify-center shadow-sm`}>
                                {stat.icon}
                            </motion.div>
                            <div>
                    ...
                    <div className="p-8 border-b border-slate-50 bg-slate-50/20 flex flex-col xl:flex-row xl:items-center justify-between gap-8">
                        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 flex-1">
                            <div className="relative flex-1 max-w-md group">
                                ...
                                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search student records..." className="w-full pl-14 pr-6 py-4 bg-white border-2 border-slate-100 focus:border-slate-900 rounded-2xl outline-none transition-all font-bold text-sm text-[#0f172a] placeholder:text-slate-300"/>
                            </div>
                            <div className="relative">
                                <select value={filterSubject} onChange={(e) => setFilterSubject(e.target.value)} className="w-full md:w-auto pl-6 pr-12 py-4 bg-[#0f172a] text-white rounded-2xl outline-none font-black text-[10px] uppercase tracking-widest appearance-none cursor-pointer hover:bg-slate-800 transition-all shadow-lg shadow-slate-200">
                        ...
                        <table className="w-full text-left">
                                ...
                                <tr className="bg-white">
                                    <th onClick={() => handleSort('name')} className="py-7 px-10 cursor-pointer group select-none relative hover:bg-slate-50 transition-colors">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="p-1 rounded-md bg-slate-50 group-hover:bg-slate-100 transition-colors">
                                                {sortConfig?.key === 'name' ? (sortConfig.direction === 'asc' ? <ChevronUp size={14} className="text-slate-900"/> : <ChevronDown size={14} className="text-slate-900"/>) : <ArrowUpDown size={12} className="text-slate-300 group-hover:text-slate-500"/>}
                                            </div>
                                        </div>
                                    </th>
                                    ...
                                    <th onClick={() => handleSort('marks')} className="py-7 px-10 cursor-pointer group text-right select-none hover:bg-slate-50 transition-colors">
                                        <div className="flex items-center justify-end gap-3">
                                            ...
                                            <div className="p-1 rounded-md bg-slate-50 group-hover:bg-slate-100 transition-colors">
                                                {sortConfig?.key === 'marks' ? (sortConfig.direction === 'asc' ? <ChevronUp size={14} className="text-slate-900"/> : <ChevronDown size={14} className="text-slate-900"/>) : <ArrowUpDown size={12} className="text-slate-300 group-hover:text-slate-500"/>}
                                            </div>
                                        </div>
                                    </th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                <AnimatePresence mode='popLayout'>
                                    {filteredData.map((student) => (<motion.tr key={student.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="group hover:bg-slate-50 transition-all duration-300">
                                            ...
                                            <td className="py-8 px-10 text-right">
                                                <div className="inline-flex flex-col items-end">
                                                    ...
                                                    <div className="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden flex shadow-inner">
                                                        <motion.div initial={{ width: 0 }} animate={{ width: `${student.marks}%` }} transition={{ duration: 1, ease: "easeOut" }} className={`h-full rounded-full ${student.marks >= 90 ? 'bg-emerald-500' :
                ...
                student.grade.startsWith('B') ? 'bg-slate-100 text-slate-900 border-slate-200' :
                    ...
                    {filteredData.length === 0 && (<div className="py-40 text-center bg-white">
