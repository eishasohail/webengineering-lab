...
export default function CounterProject() {
    ...
    return (<ProjectPageLayout title="Counter Hub" subtitle="Project #01: Counter with history log." maxWidth="max-w-2xl" showCard={false} containerClassName="bg-[#f8fafc]" backHoverColor="hover:text-blue-500">
            <div className="bg-white rounded-[3rem] p-8 sm:p-12 shadow-2xl shadow-blue-900/[0.05] border border-blue-100 relative overflow-hidden">
                ...
                <div className="relative flex flex-col items-center justify-center mb-16">
                    ...
                    <AnimatePresence mode="wait">
                        <motion.div key={count} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} className="text-8xl sm:text-9xl font-black tabular-nums text-blue-900 drop-shadow-md">
                            {count}
                        </motion.div>
                    </AnimatePresence>
                </div>
                ...
                <div className="flex items-center justify-center gap-6 mb-16">
                    <button onClick={decrement} disabled={count === 0} className="w-20 h-20 rounded-[2rem] bg-slate-50 flex items-center justify-center text-slate-500 hover:bg-rose-50 hover:text-rose-600 transition-all active:scale-90 border border-slate-200 shadow-sm disabled:opacity-30 disabled:grayscale disabled:cursor-not-allowed group" title="Decrement">
                        <Minus size={28} strokeWidth={3} className="group-hover:scale-110 transition-transform"/>
                    </button>
                    <button onClick={reset} className="w-20 h-20 rounded-[2rem] bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-white hover:text-blue-900 hover:border-blue-200 transition-all active:scale-90 border border-slate-300 shadow-sm group" title="Clear & Result History">
                        <RotateCcw size={24} strokeWidth={2.5} className="group-hover:rotate-180 transition-transform duration-500"/>
                    </button>
                    <button onClick={increment} className="w-20 h-20 rounded-[2rem] bg-blue-900 flex items-center justify-center text-white hover:bg-blue-800 transition-all active:scale-90 shadow-xl shadow-blue-900/30 group" title="Increment">
                ...
                <div className="border-t-2 border-slate-100 pt-10">
                    <div className="flex items-center justify-between mb-8 px-2">
                        ...
                        {history.length > 0 && (<button onClick={() => setHistory([])} className="text-[9px] font-black text-slate-400 hover:text-rose-500 uppercase tracking-widest transition-colors flex items-center gap-2">
                    ...
                    <div className="space-y-4">
                        <AnimatePresence initial={false} mode="popLayout">
                            {history.length === 0 ? (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-14 text-center rounded-[2.5rem] bg-slate-50/50 border border-dashed border-slate-200">
                                ...
                                </motion.div>) : (history.map((entry, i) => (<motion.div key={`${entry.timestamp}-${i}`} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="flex items-center gap-5 p-5 bg-[#fcfdfe] rounded-2xl border border-slate-200 shadow-sm hover:border-blue-200 hover:shadow-md transition-all group">
