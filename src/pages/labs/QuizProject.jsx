...
export default function QuizProject() {
  ...
  const next = () => {
    if (current < QUESTIONS.length - 1) setCurrent(current + 1);else
    setFinished(true);
  };
  ...
  return (
    <ProjectPageLayout
      title="React Quiz"
      subtitle="Project #07: Interactive quiz application."
      maxWidth="max-w-xl"
      showCard={false}
      containerClassName="bg-slate-50/50"
      backHoverColor="hover:text-[#800020]">
          ...
          <motion.div
            ...
            exit={{ opacity: 0, x: -10 }}>
                ...
                <div
                  key={i}
                  className={`h-1 rounded-full transition-all duration-500 ${i === current ? `w-6 ${BURGUNDY}` :
              ...
              <button
                ...
                className={`w-full p-4 rounded-xl border-2 transition-all flex items-center justify-between group active:scale-[0.98] ${answers[current] === idx ?
                ...
                onClick={prev}
                disabled={current === 0}
                className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${current === 0 ? "text-slate-200 cursor-not-allowed" : "text-slate-400 hover:text-[#800020]"}`
                }>
                ...
                className={`px-8 py-3.5 ${BURGUNDY} text-white rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-3 transition-all active:scale-95 shadow-lg shadow-red-900/10 ${answers[current] === -1 ? "opacity-20 cursor-not-allowed" : "hover:brightness-110"}`
                }>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col">
                            ...
                            <div className="flex justify-center gap-3 mb-10 pb-8 border-b border-slate-50">
                                {QUESTIONS.map((_, i) =>
              <div key={i} className={`w-10 h-10 rounded-xl flex items-center justify-center border-2 transition-all ${answers[i] === QUESTIONS[i].correct ?
              ...
              "bg-red-50 border-red-100 text-red-400"}`
                            ...
                            <div className="space-y-4 mb-12">
                                {QUESTIONS.map((q, i) =>
              <div key={q.id} className="flex items-center justify-between py-1 group">
                                        <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            {answers[i] !== q.correct &&
                  <span className={`${EMERALD_TEXT} text-[9px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity`}>
