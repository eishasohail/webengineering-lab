...
export default function TabDashboardProject() {
  ...
  return (
    <ProjectPageLayout
      ...
      subtitle="Project #10: Dashboard with tabs."
      maxWidth="max-w-4xl"
      backHoverColor="hover:text-teal-600">
          ...
          <button onClick={startEdit} className="flex items-center justify-center gap-2 px-6 py-4 bg-stone-50 text-stone-600 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-stone-100 transition-all border border-stone-100">
          ...
          <button
            ...
            className={`relative flex items-center gap-3 px-6 py-6 font-black text-[10px] uppercase tracking-[0.2em] transition-all ${activeTab === tab.id ? "text-teal-600" : "text-stone-300 hover:text-stone-600"}`}>
            
                            <span className={`${activeTab === tab.id ? "text-teal-600" : "text-stone-200"}`}>{tab.icon}</span>
                            <span className="hidden sm:inline">{tab.label}</span>
                            {activeTab === tab.id &&
            <motion.div layoutId="tab-underline" className="absolute bottom-0 left-6 right-6 h-0.5 bg-teal-600 rounded-full" />
            }
                        </button>
          )}
                </div>
            ...
            <motion.div key="profile" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-10">
                ...
                map((f, i) =>
                <div key={i} className="p-6 bg-[#fffafa]/30 rounded-[1.25rem] border border-stone-100 hover:border-slate-300 transition-all duration-300">
            ...
            <motion.div key="settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                ...
                map((item) => {
                  ...
                  return (
                    <div key={item.key} className="flex items-center gap-6 p-6 bg-stone-50/30 rounded-2xl border border-stone-100 transition-all group">
                        ...
                        className={`relative w-12 h-6 rounded-full transition-all duration-300 ${isOn ? "bg-teal-600" : "bg-stone-200"}`}>
            ...
            <motion.div key="orders" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                                ...
                                <div className="space-y-3">
                                    {orders.map((order) =>
                <div key={order.id} className="flex flex-col sm:flex-row items-center justify-between gap-6 p-6 bg-stone-50/30 rounded-2xl border border-stone-100 transition-all">
