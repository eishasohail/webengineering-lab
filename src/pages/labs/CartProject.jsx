...
export default function CartProject() {
  ...
  return (
    <ProjectPageLayout
      title="Beauty Bag"
      subtitle="Project #09: Shopping cart management."
      maxWidth="max-w-7xl"
      showCard={false}
      containerClassName="bg-[#fffcfc]"
      backHoverColor="hover:text-pink-500">
            ...
            <div className="flex flex-col lg:flex-row gap-8 sm:gap-12">
                ...
                <div className="flex-1">
                    <div className="space-y-4">
                        <AnimatePresence mode="popLayout">
                            {items.length === 0 ?
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="py-24 text-center bg-white rounded-[3rem] border-2 border-dashed border-rose-100 flex flex-col items-center">
                  ...
                  className="px-10 py-5 bg-[#1a1a1a] text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-rose-500 transition-all active:scale-95 shadow-lg shadow-stone-100">
              ...
              <motion.div
                ...
                className="flex flex-col sm:flex-row items-center gap-8 p-8 bg-white rounded-[2rem] border-2 border-stone-100 hover:border-rose-100 hover:shadow-xl hover:shadow-rose-900/[0.02] transition-all duration-500 relative group">
            ...
            <div className="mb-8">
                    ...
                    placeholder="Code: GLAM20"
                    className={`promo-input w-full pl-10 pr-4 py-4 bg-stone-50 border rounded-xl text-xs font-bold outline-none transition-all placeholder:text-stone-400 ${promoStatus === "error" ? "border-red-200" : "border-stone-200"}`} />
                                ...
                                {promoStatus === "error" &&
              <motion.p initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="text-[10px] font-black text-red-400 mt-3 ml-1 uppercase tracking-widest">
                                        ✗ Invalid code. Try GLAM20
                                    </motion.p>
              }
                            </div> :

            <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between p-4 bg-rose-50/50 border border-rose-100 rounded-2xl mb-8">
              ...
              <div className="flex items-center justify-between text-pink-500">
                        ...
                        <button
              onClick={() => items.length > 0 && setShowSuccess(true)}
              className={`w-full py-6 rounded-[1.25rem] font-black text-[11px] uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-3 active:scale-95 group shadow-xl ${items.length > 0 ? 'bg-[#1a1a1a] text-white hover:bg-rose-500 shadow-stone-200' : 'bg-stone-100 text-stone-300 cursor-not-allowed'}`}>
        ...
        <motion.div
          ...
          className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-stone-900/40 backdrop-blur-sm">
            ...
            className="bg-white rounded-[3rem] p-12 max-w-sm w-full text-center shadow-2xl border-4 border-rose-50">
                            ...
                            <button
              onClick={resetStore}
              className="w-full py-5 bg-[#1a1a1a] text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-rose-500 transition-all active:scale-95 shadow-lg shadow-stone-100">
